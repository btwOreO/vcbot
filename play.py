...
async def play_music_(event):
    ...
    if not jarSongs.group_call.is_connected:
        if not (await jarSongs.vc_joiner()):
            return
...
async def radio_mirchi(e):
    ...
    jarSongs = Player(chat, e)
    if not jarSongs.group_call.is_connected and not (await jarSongs.vc_joiner()):
        return
    await jarSongs.group_call.start_audio(song)
    await xx.reply(
        f"• Started Radio 📻\n\n• Station : `{song}`",
...
async def live_stream(e):
    ...
    await xx.delete()
    await jarSongs.group_call.start_audio(file)
