...
async def mute(event):
    ...
    await jarSongs.group_call.set_is_mute(True)
...
async def unmute(event):
    if len(event.text.split()) > 1:
        ...
        chat = event.chat_id
    jarSongs = Player(chat)
    await jarSongs.group_call.set_is_mute(False)
...
async def pauser(event):
    if len(event.text.split()) > 1:
        ...
        chat = event.chat_id
    jarSongs = Player(chat)
    await jarSongs.group_call.set_pause(True)
...
async def resumer(event):
    if len(event.text.split()) > 1:
        ...
        chat = event.chat_id
    jarSongs = Player(chat)
    await jarSongs.group_call.set_pause(False)
...
async def replayer(event):
    if len(event.text.split()) > 1:
        ...
        chat = event.chat_id
    jarSongs = Player(chat)
    jarSongs.group_call.restart_playout()
