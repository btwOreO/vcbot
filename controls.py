...
async def join_(event):
    if len(event.text.split()) > 1:
        ...
        chat = event.chat_id
    jarSongs = Player(chat, event)
    if not jarSongs.group_call.is_connected:
        await jarSongs.vc_joiner()
...
async def leaver(event):
    if len(event.text.split()) > 1:
        ...
        chat = event.chat_id
    jarSongs = Player(chat)
    await jarSongs.group_call.stop()
    if CLIENTS.get(chat):
        del CLIENTS[chat]
...
async def rejoiner(event):
    ...
    jarSongs = Player(chat)
    try:
        await jarSongs.group_call.reconnect()
...
async def volume_setter(event):
    ...
    if vol:
        jarSongs = Player(chat)
        await jarSongs.group_call.set_my_volume(int(vol))
        if vol > 200:
            vol = 200
        elif vol < 1:
            vol = 1
...
async def skipper(event):
    if len(event.text.split()) > 1:
        ...
        chat = event.chat_id
    jarSongs = Player(chat, event)
    await jarSongs.play_from_queue()
